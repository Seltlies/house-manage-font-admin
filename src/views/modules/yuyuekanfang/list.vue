<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"fontSize":"inherit","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">看房日期</label>
						<el-date-picker v-model="searchForm.kanfangriqistart" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="看房日期起始"></el-date-picker>
						至
						<el-date-picker v-model="searchForm.kanfangriqiend" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="看房日期结束"></el-date-picker>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yuyuekanfang','看房')" @click="kanfangxinxiCrossAddOrUpdateHandler(scope.row,'cross','否','','','')" type="success">
		...
		<kanfangxinxi-cross-add-or-update v-if="kanfangxinxiCrossAddOrUpdateFlag" :parent="this" ref="kanfangxinxiCrossaddOrUpdate"></kanfangxinxi-cross-add-or-update>
...
import kanfangxinxiCrossAddOrUpdate from "../kanfangxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				kanfangxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			kanfangxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.kanfangxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kanfangxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kanfangxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.kanfangriqistart!='' && this.searchForm.kanfangriqistart!=undefined ){
            params['kanfangriqistart'] = this.searchForm.kanfangriqistart
          }
          if(this.searchForm.kanfangriqiend!='' && this.searchForm.kanfangriqiend!=undefined){
            params['kanfangriqiend'] = this.searchForm.kanfangriqiend
          }
	...
	yuyuekanfangstatusChange(e,row){
		if(row.status==0){
