	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="专业资质" prop="zhuanyezizhi">
					<el-input v-model="ruleForm.zhuanyezizhi" placeholder="专业资质" clearable  :readonly="ro.zhuanyezizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-else class="input" label="专业资质" prop="zhuanyezizhi">
					<el-input v-model="ruleForm.zhuanyezizhi" placeholder="专业资质" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				jinglizhanghao: [
					{ required: true, message: '经理账号不能为空', trigger: 'blur' },
				],
				jinglixingming: [
					{ required: true, message: '经理姓名不能为空', trigger: 'blur' },
				],
				mima: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='zhuanyezizhi'){
							this.ruleForm.zhuanyezizhi = obj[o];
							this.ro.zhuanyezizhi = true;
							continue;
						}
				}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `fangchanjingli/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fangchanjingliCrossAddOrUpdateFlag = false;
											this.parent.search();
