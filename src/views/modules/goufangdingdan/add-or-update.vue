	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="date" v-if="type!='info'" label="购房时间" prop="goufangshijian">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.goufangshijian" 
						type="date"
						:readonly="ro.goufangshijian"
						placeholder="购房时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="input" v-else-if="ruleForm.goufangshijian" label="购房时间" prop="goufangshijian">
					<el-input v-model="ruleForm.goufangshijian" placeholder="购房时间" readonly></el-input>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.goufangshijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='goufangshijian'){
							this.ruleForm.goufangshijian = obj[o];
							this.ro.goufangshijian = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `goufangdingdan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.goufangdingdanCrossAddOrUpdateFlag = false;
											this.parent.search();
